...
def clean_dataset(filename):
    """ clean malformed lines in dataset that trips up pandas. Read in the thing one
    line at a time, and strip out the bad data. """
    ...
    with open(filename + ".cleaned", "wb") as output:
        for i, line in enumerate(open(filename)):
            ...
            # some lines contain carriage returns (without newlines), which randomly messes pandas up
            line = line.replace('\r', '')
            output.write(line)
